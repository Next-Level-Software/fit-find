...
export async function sendEmailOnRegistration(email, code) {
  try {
    ...
    template = template
      .split("||COMPANYNAME||")
      .join(companyName?.keyValue || "fitfind");
    template = template
      .split("||COMPANYWEB||")
      .join(companyWeb?.keyValue || "www.fitfind.com");
    template = template
      .split("||COMPANYAPI||")
      .join(companyAPI?.keyValue || "api.fitfind.com");
  ...
  try {
    ...
    if (htmlString) {
      ...
      if (email) {
        ...
        template = template
          .split("||COMPANYNAME||")
          .join(thisCompanyName?.keyValue || "fitfind");
        template = template
          .split("||COMPANYWEB||")
          .join(thisCompanyWeb?.keyValue || "www.fitfind.com");
        template = template
          .split("||COMPANYAPI||")
          .join(thisCompanyAPI?.keyValue || "api.fitfind.com");
